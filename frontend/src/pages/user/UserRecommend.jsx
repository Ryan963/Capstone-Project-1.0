...
export default function UserRecommend() {
  ...
  const [showFulfilledReqsModal, setShowFulfilledReqsModal] =
  ...
  const handleRecommend = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_SERVER_API}/recommendations`, config)
      .then((res) => {
        if (res.data.success) {
            ...
            <div>
              ...
              >
                <Typography variant="h6">
                  10 courses will be recommended for you to take, ranked by a
                  combination of 3 factors:
                  <p>
                    &emsp; 1. The course level (lower level gets higher
                    priority),
                  </p>
                  <p>
                    &emsp; 2. the number of requirements it helps satisfy, and
                  </p>
                  <p>
                    &emsp; 3. the number of prerequisites it satisfies for other
              ...
              <div>
                {recommendedCourses.map((course, idx) => {
                  return (
                    ...
                    >
                      <div className="w-5/6">
                        <div className="row p-2 font-bold">
                          {course.course.name}
                        </div>
                        <div className="font-bold text-sm pt-4">
                          <span>
                            This course satisfies {course.requirements.length}{" "}
                            requirement(s) and {course.prereqs.length}{" "}
                            prerequisite(s)&emsp;
                          </span>
                        </div>
                      </div>
